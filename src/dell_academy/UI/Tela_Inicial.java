/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/GUIForms/JFrame.java to edit this template
 */
package dell_academy.UI;

import java.util.ArrayList;
import java.util.List;

import javax.swing.JButton;
import javax.swing.JOptionPane;

import dell_academy.Classes.Aposta;
import dell_academy.Classes.Operador;
import dell_academy.Classes.Pessoa;

/**
 *
 * @author User
 */
public class Tela_Inicial extends javax.swing.JFrame {

    /**
     * Creates new form Tela_Inicial
     */
    List<JButton> botoesAgrupados;
    Operador operador;
    Aposta apostas;
    Pessoa pessoa;
    int[] numerosAposta;
    int numeroTotalDeApostas;

    public Tela_Inicial() {
        initComponents();
        setVisible(true);
        this.setResizable(false);
        this.setTitle("MENU DELL-SENA");
        botoesAgrupados = new ArrayList<>();
        adicionarBotoes();
        numerosAposta = new int[5];
        operador = new Operador();
        pessoa = new Pessoa(null, null);
        this.pack();
        this.setLocationRelativeTo(null);
    }

    private void adicionarBotoes() {
        botoesAgrupados.add(btn01);
        botoesAgrupados.add(btn02);
        botoesAgrupados.add(btn03);
        botoesAgrupados.add(btn04);
        botoesAgrupados.add(btn05);
        botoesAgrupados.add(btn06);
        botoesAgrupados.add(btn07);
        botoesAgrupados.add(btn08);
        botoesAgrupados.add(btn09);
        botoesAgrupados.add(btn10);
        botoesAgrupados.add(btn11);
        botoesAgrupados.add(btn12);
        botoesAgrupados.add(btn13);
        botoesAgrupados.add(btn14);
        botoesAgrupados.add(btn15);
        botoesAgrupados.add(btn16);
        botoesAgrupados.add(btn17);
        botoesAgrupados.add(btn18);
        botoesAgrupados.add(btn19);
        botoesAgrupados.add(btn20);
        botoesAgrupados.add(btn21);
        botoesAgrupados.add(btn22);
        botoesAgrupados.add(btn23);
        botoesAgrupados.add(btn24);
        botoesAgrupados.add(btn25);
        botoesAgrupados.add(btn26);
        botoesAgrupados.add(btn27);
        botoesAgrupados.add(btn28);
        botoesAgrupados.add(btn29);
        botoesAgrupados.add(btn30);
        botoesAgrupados.add(btn31);
        botoesAgrupados.add(btn32);
        botoesAgrupados.add(btn33);
        botoesAgrupados.add(btn34);
        botoesAgrupados.add(btn35);
        botoesAgrupados.add(btn36);
        botoesAgrupados.add(btn37);
        botoesAgrupados.add(btn38);
        botoesAgrupados.add(btn39);
        botoesAgrupados.add(btn40);
        botoesAgrupados.add(btn41);
        botoesAgrupados.add(btn42);
        botoesAgrupados.add(btn43);
        botoesAgrupados.add(btn44);
        botoesAgrupados.add(btn45);
        botoesAgrupados.add(btn46);
        botoesAgrupados.add(btn47);
        botoesAgrupados.add(btn48);
        botoesAgrupados.add(btn49);
        botoesAgrupados.add(btn50);
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    // <editor-fold defaultstate="collapsed" desc="Generated
    // <editor-fold defaultstate="collapsed" desc="Generated
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        jButton17 = new javax.swing.JButton();
        jButton26 = new javax.swing.JButton();
        jButton51 = new javax.swing.JButton();
        jLabel1 = new javax.swing.JLabel();
        jLabel2 = new javax.swing.JLabel();
        btnIniciar = new javax.swing.JButton();
        jLabel3 = new javax.swing.JLabel();
        campoNome = new javax.swing.JTextField();
        jLabel4 = new javax.swing.JLabel();
        campoCPF = new javax.swing.JTextField();
        btn03 = new javax.swing.JButton();
        btn04 = new javax.swing.JButton();
        btn05 = new javax.swing.JButton();
        btn06 = new javax.swing.JButton();
        btn07 = new javax.swing.JButton();
        btn08 = new javax.swing.JButton();
        btn09 = new javax.swing.JButton();
        btn10 = new javax.swing.JButton();
        btn11 = new javax.swing.JButton();
        btn12 = new javax.swing.JButton();
        btn13 = new javax.swing.JButton();
        btn14 = new javax.swing.JButton();
        btn15 = new javax.swing.JButton();
        btn16 = new javax.swing.JButton();
        btn17 = new javax.swing.JButton();
        btn18 = new javax.swing.JButton();
        btn19 = new javax.swing.JButton();
        btn20 = new javax.swing.JButton();
        btn21 = new javax.swing.JButton();
        btn22 = new javax.swing.JButton();
        btn23 = new javax.swing.JButton();
        btn24 = new javax.swing.JButton();
        btn25 = new javax.swing.JButton();
        btn26 = new javax.swing.JButton();
        btn27 = new javax.swing.JButton();
        btn28 = new javax.swing.JButton();
        btn29 = new javax.swing.JButton();
        btn30 = new javax.swing.JButton();
        btn31 = new javax.swing.JButton();
        btn32 = new javax.swing.JButton();
        btn33 = new javax.swing.JButton();
        btn34 = new javax.swing.JButton();
        btn35 = new javax.swing.JButton();
        btn36 = new javax.swing.JButton();
        btn37 = new javax.swing.JButton();
        btn38 = new javax.swing.JButton();
        btn39 = new javax.swing.JButton();
        btn40 = new javax.swing.JButton();
        btn41 = new javax.swing.JButton();
        btn42 = new javax.swing.JButton();
        btn43 = new javax.swing.JButton();
        btn44 = new javax.swing.JButton();
        btn45 = new javax.swing.JButton();
        btn46 = new javax.swing.JButton();
        btn47 = new javax.swing.JButton();
        btn48 = new javax.swing.JButton();
        btn49 = new javax.swing.JButton();
        btn50 = new javax.swing.JButton();
        btn02 = new javax.swing.JButton();
        btn01 = new javax.swing.JButton();
        btnApostar = new javax.swing.JButton();
        btnListar = new javax.swing.JButton();
        btnSorteio = new javax.swing.JButton();
        comboNomes = new javax.swing.JComboBox<>();
        btnAddPessoa = new javax.swing.JButton();
        jLabel5 = new javax.swing.JLabel();
        checkSurpresa = new javax.swing.JCheckBox();
        btnLimpar = new javax.swing.JButton();

        jButton17.setText("01");
        jButton17.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton17ActionPerformed(evt);
            }
        });

        jButton26.setText("01");
        jButton26.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton26ActionPerformed(evt);
            }
        });

        jButton51.setText("01");
        jButton51.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton51ActionPerformed(evt);
            }
        });

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);

        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/dell_academy/UI/trevo-esc-peq.png"))); // NOI18N
        jLabel1.setText("jLabel1");

        jLabel2.setFont(new java.awt.Font("Segoe UI", 1, 24)); // NOI18N
        jLabel2.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        jLabel2.setText("DELL-SENA");

        btnIniciar.setBackground(new java.awt.Color(0, 153, 51));
        btnIniciar.setFont(new java.awt.Font("Segoe UI", 1, 14)); // NOI18N
        btnIniciar.setText("INICIAR");
        btnIniciar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnIniciarActionPerformed(evt);
            }
        });

        jLabel3.setText("NOME:");

        campoNome.setEnabled(false);

        jLabel4.setText("CPF:");

        campoCPF.setEnabled(false);

        btn03.setText("03");
        btn03.setEnabled(false);
        btn03.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn03ActionPerformed(evt);
            }
        });

        btn04.setText("04");
        btn04.setEnabled(false);
        btn04.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn04ActionPerformed(evt);
            }
        });

        btn05.setText("05");
        btn05.setEnabled(false);
        btn05.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn05ActionPerformed(evt);
            }
        });

        btn06.setText("06");
        btn06.setEnabled(false);
        btn06.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn06ActionPerformed(evt);
            }
        });

        btn07.setText("07");
        btn07.setEnabled(false);
        btn07.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn07ActionPerformed(evt);
            }
        });

        btn08.setText("08");
        btn08.setEnabled(false);
        btn08.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn08ActionPerformed(evt);
            }
        });

        btn09.setText("09");
        btn09.setEnabled(false);
        btn09.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn09ActionPerformed(evt);
            }
        });

        btn10.setText("10");
        btn10.setEnabled(false);
        btn10.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn10ActionPerformed(evt);
            }
        });

        btn11.setText("11");
        btn11.setEnabled(false);
        btn11.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn11ActionPerformed(evt);
            }
        });

        btn12.setText("12");
        btn12.setEnabled(false);
        btn12.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn12ActionPerformed(evt);
            }
        });

        btn13.setText("13");
        btn13.setEnabled(false);
        btn13.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn13ActionPerformed(evt);
            }
        });

        btn14.setText("14");
        btn14.setEnabled(false);
        btn14.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn14ActionPerformed(evt);
            }
        });

        btn15.setText("15");
        btn15.setEnabled(false);
        btn15.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn15ActionPerformed(evt);
            }
        });

        btn16.setText("16");
        btn16.setEnabled(false);
        btn16.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn16ActionPerformed(evt);
            }
        });

        btn17.setText("17");
        btn17.setEnabled(false);
        btn17.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn17ActionPerformed(evt);
            }
        });

        btn18.setText("18");
        btn18.setEnabled(false);
        btn18.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn18ActionPerformed(evt);
            }
        });

        btn19.setText("19");
        btn19.setEnabled(false);
        btn19.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn19ActionPerformed(evt);
            }
        });

        btn20.setText("20");
        btn20.setEnabled(false);
        btn20.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn20ActionPerformed(evt);
            }
        });

        btn21.setText("21");
        btn21.setEnabled(false);
        btn21.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn21ActionPerformed(evt);
            }
        });

        btn22.setText("22");
        btn22.setEnabled(false);
        btn22.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn22ActionPerformed(evt);
            }
        });

        btn23.setText("23");
        btn23.setEnabled(false);
        btn23.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn23ActionPerformed(evt);
            }
        });

        btn24.setText("24");
        btn24.setEnabled(false);
        btn24.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn24ActionPerformed(evt);
            }
        });

        btn25.setText("25");
        btn25.setEnabled(false);
        btn25.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn25ActionPerformed(evt);
            }
        });

        btn26.setText("26");
        btn26.setEnabled(false);
        btn26.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn26ActionPerformed(evt);
            }
        });

        btn27.setText("27");
        btn27.setEnabled(false);
        btn27.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn27ActionPerformed(evt);
            }
        });

        btn28.setText("28");
        btn28.setEnabled(false);
        btn28.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn28ActionPerformed(evt);
            }
        });

        btn29.setText("29");
        btn29.setEnabled(false);
        btn29.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn29ActionPerformed(evt);
            }
        });

        btn30.setText("30");
        btn30.setEnabled(false);
        btn30.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn30ActionPerformed(evt);
            }
        });

        btn31.setText("31");
        btn31.setEnabled(false);
        btn31.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn31ActionPerformed(evt);
            }
        });

        btn32.setText("32");
        btn32.setEnabled(false);
        btn32.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn32ActionPerformed(evt);
            }
        });

        btn33.setText("33");
        btn33.setEnabled(false);
        btn33.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn33ActionPerformed(evt);
            }
        });

        btn34.setText("34");
        btn34.setEnabled(false);
        btn34.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn34ActionPerformed(evt);
            }
        });

        btn35.setText("35");
        btn35.setEnabled(false);
        btn35.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn35ActionPerformed(evt);
            }
        });

        btn36.setText("36");
        btn36.setEnabled(false);
        btn36.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn36ActionPerformed(evt);
            }
        });

        btn37.setText("37");
        btn37.setEnabled(false);
        btn37.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn37ActionPerformed(evt);
            }
        });

        btn38.setText("38");
        btn38.setEnabled(false);
        btn38.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn38ActionPerformed(evt);
            }
        });

        btn39.setText("39");
        btn39.setEnabled(false);
        btn39.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn39ActionPerformed(evt);
            }
        });

        btn40.setText("40");
        btn40.setEnabled(false);
        btn40.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn40ActionPerformed(evt);
            }
        });

        btn41.setText("41");
        btn41.setEnabled(false);
        btn41.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn41ActionPerformed(evt);
            }
        });

        btn42.setText("42");
        btn42.setEnabled(false);
        btn42.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn42ActionPerformed(evt);
            }
        });

        btn43.setText("43");
        btn43.setEnabled(false);
        btn43.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn43ActionPerformed(evt);
            }
        });

        btn44.setText("44");
        btn44.setEnabled(false);
        btn44.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn44ActionPerformed(evt);
            }
        });

        btn45.setText("45");
        btn45.setEnabled(false);
        btn45.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn45ActionPerformed(evt);
            }
        });

        btn46.setText("46");
        btn46.setEnabled(false);
        btn46.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn46ActionPerformed(evt);
            }
        });

        btn47.setText("47");
        btn47.setEnabled(false);
        btn47.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn47ActionPerformed(evt);
            }
        });

        btn48.setText("48");
        btn48.setEnabled(false);
        btn48.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn48ActionPerformed(evt);
            }
        });

        btn49.setText("49");
        btn49.setEnabled(false);
        btn49.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn49ActionPerformed(evt);
            }
        });

        btn50.setText("50");
        btn50.setEnabled(false);
        btn50.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn50ActionPerformed(evt);
            }
        });

        btn02.setText("02");
        btn02.setEnabled(false);
        btn02.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn02ActionPerformed(evt);
            }
        });

        btn01.setText("01");
        btn01.setEnabled(false);
        btn01.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn01ActionPerformed(evt);
            }
        });

        btnApostar.setText("APOSTAR");
        btnApostar.setEnabled(false);
        btnApostar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnApostarActionPerformed(evt);
            }
        });

        btnListar.setText("LISTAR");
        btnListar.setEnabled(false);
        btnListar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnListarActionPerformed(evt);
            }
        });

        btnSorteio.setText("SORTEIO");
        btnSorteio.setEnabled(false);
        btnSorteio.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnSorteioActionPerformed(evt);
            }
        });

        comboNomes.setEnabled(false);
        comboNomes.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                comboNomesActionPerformed(evt);
            }
        });

        btnAddPessoa.setText("ADICIONAR PESSOA");
        btnAddPessoa.setEnabled(false);
        btnAddPessoa.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnAddPessoaActionPerformed(evt);
            }
        });

        jLabel5.setText("SELECIONAR APOSTADOR");

        checkSurpresa.setText("SURPRESA");
        checkSurpresa.setEnabled(false);
        checkSurpresa.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                checkSurpresaActionPerformed(evt);
            }
        });

        btnLimpar.setText("LIMPAR");
        btnLimpar.setEnabled(false);
        btnLimpar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnLimparActionPerformed(evt);
            }
        });

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(getContentPane());
        getContentPane().setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(layout.createSequentialGroup()
                .addGap(38, 38, 38)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(jLabel5)
                        .addGap(0, 0, Short.MAX_VALUE))
                    .addGroup(layout.createSequentialGroup()
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(jLabel4)
                                .addGap(18, 18, 18))
                            .addGroup(javax.swing.GroupLayout.Alignment.LEADING, layout.createSequentialGroup()
                                .addComponent(comboNomes, javax.swing.GroupLayout.PREFERRED_SIZE, 226, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)))
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            .addComponent(btnApostar, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                            .addComponent(campoCPF, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, 164, Short.MAX_VALUE)
                            .addComponent(btnAddPessoa, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                        .addGap(37, 37, 37))
                    .addGroup(layout.createSequentialGroup()
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(jLabel1, javax.swing.GroupLayout.PREFERRED_SIZE, 132, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addGap(31, 31, 31)
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addComponent(jLabel2, javax.swing.GroupLayout.PREFERRED_SIZE, 171, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addComponent(btnIniciar, javax.swing.GroupLayout.PREFERRED_SIZE, 171, javax.swing.GroupLayout.PREFERRED_SIZE)))
                            .addGroup(layout.createSequentialGroup()
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                    .addGroup(layout.createSequentialGroup()
                                        .addComponent(btn43, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                        .addComponent(btn44, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE))
                                    .addComponent(btnLimpar, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn45, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                    .addGroup(layout.createSequentialGroup()
                                        .addComponent(btn46, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                        .addComponent(btn47, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE))
                                    .addComponent(btnListar, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn48, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addGroup(layout.createSequentialGroup()
                                        .addComponent(btn49, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                        .addComponent(btn50, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE))
                                    .addComponent(btnSorteio, javax.swing.GroupLayout.PREFERRED_SIZE, 116, javax.swing.GroupLayout.PREFERRED_SIZE)))
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(btn35, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn36, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn37, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn38, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn39, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn40, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn41, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn42, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE))
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(btn27, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn28, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn29, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn30, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn31, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn32, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn33, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn34, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE))
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(btn19, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn20, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn21, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn22, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn23, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn24, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn25, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(btn26, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE))
                            .addGroup(layout.createSequentialGroup()
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                                    .addGroup(layout.createSequentialGroup()
                                        .addComponent(btn03, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                        .addComponent(btn04, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addGap(12, 12, 12)
                                        .addComponent(btn05, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                        .addComponent(btn06, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE))
                                    .addComponent(btn01, javax.swing.GroupLayout.PREFERRED_SIZE, 53, javax.swing.GroupLayout.PREFERRED_SIZE))
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                                    .addComponent(btn02, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addComponent(btn07, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)))
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(jLabel3)
                                .addGap(14, 14, 14)
                                .addComponent(campoNome, javax.swing.GroupLayout.PREFERRED_SIZE, 174, javax.swing.GroupLayout.PREFERRED_SIZE))
                            .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                                .addComponent(checkSurpresa)
                                .addGroup(layout.createSequentialGroup()
                                    .addComponent(btn11, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addGap(12, 12, 12)
                                    .addComponent(btn12, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addGap(12, 12, 12)
                                    .addComponent(btn13, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                    .addComponent(btn14, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addGap(12, 12, 12)
                                    .addComponent(btn15, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addGap(12, 12, 12)
                                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                        .addGroup(layout.createSequentialGroup()
                                            .addComponent(btn08, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                            .addComponent(btn09, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                            .addComponent(btn10, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE))
                                        .addGroup(layout.createSequentialGroup()
                                            .addComponent(btn16, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                            .addComponent(btn17, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                            .addComponent(btn18, javax.swing.GroupLayout.PREFERRED_SIZE, 52, javax.swing.GroupLayout.PREFERRED_SIZE))))))
                        .addContainerGap(35, Short.MAX_VALUE))))
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(btnIniciar, javax.swing.GroupLayout.Alignment.TRAILING)
                    .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                        .addComponent(jLabel1, javax.swing.GroupLayout.PREFERRED_SIZE, 152, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addComponent(jLabel2, javax.swing.GroupLayout.PREFERRED_SIZE, 85, javax.swing.GroupLayout.PREFERRED_SIZE)))
                .addGap(29, 29, 29)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(campoNome, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(jLabel3)
                    .addComponent(jLabel4)
                    .addComponent(campoCPF, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(btnAddPessoa)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, 39, Short.MAX_VALUE)
                .addComponent(jLabel5)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(comboNomes, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(btnApostar))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addComponent(checkSurpresa)
                .addGap(7, 7, 7)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(btn02)
                    .addComponent(btn01))
                .addGap(18, 18, 18)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(btn04)
                    .addComponent(btn05)
                    .addComponent(btn06)
                    .addComponent(btn07)
                    .addComponent(btn08)
                    .addComponent(btn09)
                    .addComponent(btn10)
                    .addComponent(btn03))
                .addGap(18, 18, 18)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(btn11)
                    .addComponent(btn12)
                    .addComponent(btn13)
                    .addComponent(btn14)
                    .addComponent(btn15)
                    .addComponent(btn16)
                    .addComponent(btn17)
                    .addComponent(btn18))
                .addGap(18, 18, 18)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(btn19)
                    .addComponent(btn20)
                    .addComponent(btn21)
                    .addComponent(btn22)
                    .addComponent(btn23)
                    .addComponent(btn24)
                    .addComponent(btn25)
                    .addComponent(btn26))
                .addGap(18, 18, 18)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(btn27)
                    .addComponent(btn28)
                    .addComponent(btn29)
                    .addComponent(btn30)
                    .addComponent(btn31)
                    .addComponent(btn32)
                    .addComponent(btn33)
                    .addComponent(btn34))
                .addGap(18, 18, 18)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(btn35)
                    .addComponent(btn36)
                    .addComponent(btn37)
                    .addComponent(btn38)
                    .addComponent(btn39)
                    .addComponent(btn40)
                    .addComponent(btn41)
                    .addComponent(btn42))
                .addGap(18, 18, 18)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(btn43)
                    .addComponent(btn44)
                    .addComponent(btn45)
                    .addComponent(btn46)
                    .addComponent(btn47)
                    .addComponent(btn48)
                    .addComponent(btn49)
                    .addComponent(btn50))
                .addGap(18, 18, 18)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(btnListar)
                    .addComponent(btnSorteio)
                    .addComponent(btnLimpar))
                .addGap(19, 19, 19))
        );

        pack();
    }// </editor-fold>//GEN-END:initComponents

    private void btnLimparActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLimparActionPerformed
        for(JButton botoes : botoesAgrupados) {
            botoes.setSelected(false);
            botoes.setEnabled(true);
            botoes.setBackground(null);
        }
        checkSurpresa.setEnabled(true);
        checkSurpresa.setSelected(false);
        numerosAposta = new int[5];
    }//GEN-LAST:event_btnLimparActionPerformed

    private void btnSorteioActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnSorteioActionPerformed
        new ConfirmacaoSorteio(operador, this);
    }// GEN-LAST:event_btnSorteioActionPerformed

    private void btn03ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn03ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn03, numerosAposta);
            operador.setClicado(btn03);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn03ActionPerformed

    private void btn04ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn04ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn04, numerosAposta);
            operador.setClicado(btn04);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn04ActionPerformed

    private void btn05ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn05ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn05, numerosAposta);
            operador.setClicado(btn05);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn05ActionPerformed

    private void btn06ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn06ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn06, numerosAposta);
            operador.setClicado(btn06);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn06ActionPerformed

    private void btn07ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn07ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn07, numerosAposta);
            operador.setClicado(btn07);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn07ActionPerformed

    private void btn08ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn08ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn08, numerosAposta);
            operador.setClicado(btn08);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn08ActionPerformed

    private void btn09ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn09ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn09, numerosAposta);
            operador.setClicado(btn09);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn09ActionPerformed

    private void btn10ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn10ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn10, numerosAposta);
            operador.setClicado(btn10);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn10ActionPerformed

    private void btn11ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn11ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn11, numerosAposta);
            operador.setClicado(btn11);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn11ActionPerformed

    private void btn12ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn12ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn12, numerosAposta);
            operador.setClicado(btn12);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn12ActionPerformed

    private void btn13ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn13ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn13, numerosAposta);
            operador.setClicado(btn13);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn13ActionPerformed

    private void btn14ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn14ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn14, numerosAposta);
            operador.setClicado(btn14);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn14ActionPerformed

    private void btn15ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn15ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn15, numerosAposta);
            operador.setClicado(btn15);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn15ActionPerformed

    private void btn16ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn16ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn16, numerosAposta);
            operador.setClicado(btn16);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn16ActionPerformed

    private void btn17ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn17ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn17, numerosAposta);
            operador.setClicado(btn17);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn17ActionPerformed

    private void btn18ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn18ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn18, numerosAposta);
            operador.setClicado(btn18);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn18ActionPerformed

    private void jButton17ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButton17ActionPerformed
    }// GEN-LAST:event_jButton17ActionPerformed

    private void btn19ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn19ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn19, numerosAposta);
            operador.setClicado(btn19);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn19ActionPerformed

    private void btn20ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn20ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn20, numerosAposta);
            operador.setClicado(btn20);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn20ActionPerformed

    private void btn21ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn21ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn21, numerosAposta);
            operador.setClicado(btn21);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn21ActionPerformed

    private void btn22ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn22ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn22, numerosAposta);
            operador.setClicado(btn22);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn22ActionPerformed

    private void btn23ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn23ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn23, numerosAposta);
            operador.setClicado(btn23);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn23ActionPerformed

    private void btn24ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn24ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn24, numerosAposta);
            operador.setClicado(btn24);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn24ActionPerformed

    private void btn25ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn25ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn25, numerosAposta);
            operador.setClicado(btn25);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn25ActionPerformed

    private void btn26ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn26ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn26, numerosAposta);
            operador.setClicado(btn26);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn26ActionPerformed

    private void jButton26ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButton26ActionPerformed

    }// GEN-LAST:event_jButton26ActionPerformed

    private void btn27ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn27ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn27, numerosAposta);
            operador.setClicado(btn27);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn27ActionPerformed

    private void btn28ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn28ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn28, numerosAposta);
            operador.setClicado(btn28);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn28ActionPerformed

    private void btn29ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn29ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn29, numerosAposta);
            operador.setClicado(btn29);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn29ActionPerformed

    private void btn30ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn30ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn30, numerosAposta);
            operador.setClicado(btn30);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn30ActionPerformed

    private void btn31ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn31ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn31, numerosAposta);
            operador.setClicado(btn31);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn31ActionPerformed

    private void btn32ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn32ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn32, numerosAposta);
            operador.setClicado(btn32);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn32ActionPerformed

    private void btn33ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn33ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn33, numerosAposta);
            operador.setClicado(btn33);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn33ActionPerformed

    private void btn34ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn34ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn34, numerosAposta);
            operador.setClicado(btn34);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn34ActionPerformed

    private void btn35ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn35ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn35, numerosAposta);
            operador.setClicado(btn35);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn35ActionPerformed

    private void btn36ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn36ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn36, numerosAposta);
            operador.setClicado(btn36);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn36ActionPerformed

    private void btn37ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn37ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn37, numerosAposta);
            operador.setClicado(btn37);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn37ActionPerformed

    private void btn38ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn38ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn38, numerosAposta);
            operador.setClicado(btn38);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn38ActionPerformed

    private void btn39ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn39ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn39, numerosAposta);
            operador.setClicado(btn39);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn39ActionPerformed

    private void btn40ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn40ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn40, numerosAposta);
            operador.setClicado(btn40);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn40ActionPerformed

    private void btn41ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn41ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn41, numerosAposta);
            operador.setClicado(btn41);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn41ActionPerformed

    private void btn42ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn42ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn42, numerosAposta);
            operador.setClicado(btn42);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn42ActionPerformed

    private void btn43ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn43ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn43, numerosAposta);
            operador.setClicado(btn43);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn43ActionPerformed

    private void btn44ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn44ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn44, numerosAposta);
            operador.setClicado(btn44);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn44ActionPerformed

    private void btn45ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn45ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn45, numerosAposta);
            operador.setClicado(btn45);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn45ActionPerformed

    private void btn46ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn46ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn46, numerosAposta);
            operador.setClicado(btn46);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn46ActionPerformed

    private void btn47ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn47ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn47, numerosAposta);
            operador.setClicado(btn47);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn47ActionPerformed

    private void btn48ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn48ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn48, numerosAposta);
            operador.setClicado(btn48);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn48ActionPerformed

    private void btn49ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn49ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn49, numerosAposta);
            operador.setClicado(btn49);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn49ActionPerformed

    private void btn50ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn50ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn50, numerosAposta);
            operador.setClicado(btn50);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn50ActionPerformed

    private void jButton51ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButton51ActionPerformed
    }// GEN-LAST:event_jButton51ActionPerformed

    private void btn02ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn02ActionPerformed
        checkSurpresa.setEnabled(false);
        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn02, numerosAposta);
            operador.setClicado(btn02);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn02ActionPerformed

    private void btn01ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn01ActionPerformed
        
        checkSurpresa.setEnabled(false);
        

        if (!operador.verificarSeVetorEstaCheio(numerosAposta)) {
            operador.adicionarValorArray(btn01, numerosAposta);
            operador.setClicado(btn01);

            if (operador.verificarSeVetorEstaCheio(numerosAposta)) {
                for (JButton botao : botoesAgrupados) {
                    botao.setEnabled(false);
                }
            }
        }
    }// GEN-LAST:event_btn01ActionPerformed

    private void btnIniciarActionPerformed(java.awt.event.ActionEvent evt) {
        for (JButton botao : botoesAgrupados) {
            botao.setEnabled(true);
        }
        btnIniciar.setEnabled(false);
        btnLimpar.setEnabled(true);
        campoNome.setEnabled(true);
        campoCPF.setEnabled(true);
        btnApostar.setEnabled(true);
        checkSurpresa.setEnabled(true);
        btnAddPessoa.setEnabled(true);
        comboNomes.setEnabled(true);

    }

    private void btnApostarActionPerformed(java.awt.event.ActionEvent evt) {

        if (operador.getListaPessoas().isEmpty()) {
            JOptionPane.showMessageDialog(null, "NÃO HÁ PESSOAS CRIADAS PARA REALIZAR APOSTA.");
            for (JButton botoes : botoesAgrupados) {
                operador.setZerado(botoes);
            }
            numerosAposta = new int[5];
            checkSurpresa.setSelected(false);
            return; // Encerra o método apenas se não houver pessoas
        } else {
            checkSurpresa.setEnabled(true);
            int pessoaSelecionada = comboNomes.getSelectedIndex();
            Pessoa selecionada = operador.getListaPessoas().get(pessoaSelecionada);

            // Verifica se a aposta surpresa foi selecionada
            if (checkSurpresa.isSelected()) {
                operador.apostaSurpresa(selecionada);
                JOptionPane.showMessageDialog(null, "APOSTA SURPRESA REALIZADA COM SUCESSO!");
            } else {
                // Verifica se pelo menos um número foi selecionado
                boolean todosNumerosSelecionados = true;
                for (int numero : numerosAposta) {
                    if (numero == 0) {
                        todosNumerosSelecionados = false;
                        break;
                    }
                }

                // Se algum número da aposta for zero, exibe uma mensagem e encerra o método
                if (!todosNumerosSelecionados) {
                    JOptionPane.showMessageDialog(null, "APOSTA NÃO PODE CONTER MENOS DE 5 NÚMEROS.");
                    for (JButton botoes : botoesAgrupados) {
                        operador.setZerado(botoes);
                        numerosAposta = new int[5];
                    }
                    return;
                }
                boolean numerosSelecionados = false;
                for (int numero : numerosAposta) {
                    if (numero != 0) {
                        numerosSelecionados = true;
                        break;
                    }
                }

                if (!numerosSelecionados) {
                    JOptionPane.showMessageDialog(null, "PROIBIDO REALIZAR APOSTA SEM NÚMEROS!");
                    return; // Encerra o método se não houver números na aposta
                }

                Aposta novaAposta = new Aposta(selecionada, numerosAposta);
                selecionada.adicionarAposta(novaAposta);
                operador.adicionarApostasArray(novaAposta);

                JOptionPane.showMessageDialog(null, "APOSTA REALIZADA COM SUCESSO!");

                // Reset do vetor de números da aposta
                numerosAposta = new int[5];

                // Limpa os valores nos campos de entrada de números
                for (JButton botoes : botoesAgrupados) {
                    operador.setZerado(botoes);
                }
            }

            // Habilitar os botões necessários
            btnSorteio.setEnabled(true);
            btnListar.setEnabled(true);
        }
    }

    private void btnAddPessoaActionPerformed(java.awt.event.ActionEvent evt) {
        String nome = campoNome.getText().trim();
        String cpf = campoCPF.getText().trim();
        checkSurpresa.setEnabled(true);

        // Verifica se o nome e o CPF não estão vazios
        if (nome.isEmpty() || cpf.isEmpty()) {
            JOptionPane.showMessageDialog(null, "POR FAVOR, INSIRA O NOME E O CPF DA PESSOA.");
            return; // Encerra o método se o nome ou o CPF estiverem vazios
        }

        // Verifica se o nome contém apenas letras
        if (!nome.matches("[a-zA-ZÀ-ÖØ-öø-ÿ\\s]+") || !cpf.matches("\\d{11}")) {
            JOptionPane.showMessageDialog(null, "NOME OU CPF INVÁLIDOS");
            campoCPF.setText("");
            campoNome.setText("");
            return; // Encerra o método se o nome não contiver apenas letras
        }

        // Cria uma nova Pessoa com o nome e o CPF informados
        Pessoa novaPessoa = new Pessoa(nome, cpf);
        operador.adicionarPessoa(novaPessoa);
        comboNomes.setModel(new javax.swing.DefaultComboBoxModel<>(operador.listarPessoas()));

        JOptionPane.showMessageDialog(null, "PESSOA ADICIONADA COM SUCESSO!");

        campoNome.setText("");
        campoCPF.setText("");
    }

    private void comboNomesActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_comboNomesActionPerformed
    }// GEN-LAST:event_comboNomesActionPerformed

    private void checkSurpresaActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_checkSurpresaActionPerformed
        if (checkSurpresa.isSelected()) {
            for (JButton botoes : botoesAgrupados) {
                botoes.setEnabled(false);
            }
        } else {
            for (JButton botoes : botoesAgrupados) {
                botoes.setEnabled(true);
            }
        }
    }// GEN-LAST:event_checkSurpresaActionPerformed

    private void btnListarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnListarActionPerformed
        new ListaApostas(operador);
    }// GEN-LAST:event_btnListarActionPerformed

    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton btn01;
    private javax.swing.JButton btn02;
    private javax.swing.JButton btn03;
    private javax.swing.JButton btn04;
    private javax.swing.JButton btn05;
    private javax.swing.JButton btn06;
    private javax.swing.JButton btn07;
    private javax.swing.JButton btn08;
    private javax.swing.JButton btn09;
    private javax.swing.JButton btn10;
    private javax.swing.JButton btn11;
    private javax.swing.JButton btn12;
    private javax.swing.JButton btn13;
    private javax.swing.JButton btn14;
    private javax.swing.JButton btn15;
    private javax.swing.JButton btn16;
    private javax.swing.JButton btn17;
    private javax.swing.JButton btn18;
    private javax.swing.JButton btn19;
    private javax.swing.JButton btn20;
    private javax.swing.JButton btn21;
    private javax.swing.JButton btn22;
    private javax.swing.JButton btn23;
    private javax.swing.JButton btn24;
    private javax.swing.JButton btn25;
    private javax.swing.JButton btn26;
    private javax.swing.JButton btn27;
    private javax.swing.JButton btn28;
    private javax.swing.JButton btn29;
    private javax.swing.JButton btn30;
    private javax.swing.JButton btn31;
    private javax.swing.JButton btn32;
    private javax.swing.JButton btn33;
    private javax.swing.JButton btn34;
    private javax.swing.JButton btn35;
    private javax.swing.JButton btn36;
    private javax.swing.JButton btn37;
    private javax.swing.JButton btn38;
    private javax.swing.JButton btn39;
    private javax.swing.JButton btn40;
    private javax.swing.JButton btn41;
    private javax.swing.JButton btn42;
    private javax.swing.JButton btn43;
    private javax.swing.JButton btn44;
    private javax.swing.JButton btn45;
    private javax.swing.JButton btn46;
    private javax.swing.JButton btn47;
    private javax.swing.JButton btn48;
    private javax.swing.JButton btn49;
    private javax.swing.JButton btn50;
    private javax.swing.JButton btnAddPessoa;
    private javax.swing.JButton btnApostar;
    private javax.swing.JButton btnIniciar;
    private javax.swing.JButton btnLimpar;
    private javax.swing.JButton btnListar;
    private javax.swing.JButton btnSorteio;
    private javax.swing.JTextField campoCPF;
    private javax.swing.JTextField campoNome;
    private javax.swing.JCheckBox checkSurpresa;
    private javax.swing.JComboBox<String> comboNomes;
    private javax.swing.JButton jButton17;
    private javax.swing.JButton jButton26;
    private javax.swing.JButton jButton51;
    private javax.swing.JLabel jLabel1;
    private javax.swing.JLabel jLabel2;
    private javax.swing.JLabel jLabel3;
    private javax.swing.JLabel jLabel4;
    private javax.swing.JLabel jLabel5;
    // End of variables declaration//GEN-END:variables
}
